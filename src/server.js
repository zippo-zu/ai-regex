...
const configuration = new Configuration({
  apiKey: process.env.CHATBOT_KEY,basePath:process.env.BASE_URL
});
...
app.post("/v1/completions", async (req, res) => {
  const { sampleText, target } = req.body;
  const completion = await openai.createCompletion({
    model: "text-davinci-003", 
    prompt: `Write a Regex expression for the following text "${sampleText}"
